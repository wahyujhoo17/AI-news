...
function IdHomeContent() {
  ...
  const mainArticles = remainingWithImage.slice(0, 6)

  const usedInCards = new Set(mainArticles.map((article) => article.id))
  const sidebarArticles = [
    ...remainingWithoutImage,
    ...remainingWithImage.filter((article) => !usedInCards.has(article.id)),
  ].slice(0, 4)
  ...
  const handleCategoryChange = (slug: string) => {
    ...
    setSearch("")
    setDebouncedSearch("")
    router.push(slug ? `/id?category=${slug}` : "/id")
  }
  ...
  const handlePageChange = (page: number) => {
    ...
    if (!isSearchMode) {
      router.push(`/id?page=${page}${selectedCategory ? `&category=${selectedCategory}` : ""}`)
    }
  }
          ...
          <>
            {featured.length > 0 && (
              <div className="mb-12">
                ...
                <Link
                  href={buildIdArticlePath(featured[0].id, featured[0].title)}
            ...
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-8 mb-12">
              ...
              {sidebarArticles.length > 0 && (
                <div className="lg:col-span-1">
                  <h2 className="text-sm font-bold text-cyan-400 uppercase tracking-wider mb-4">Berita Lainnya</h2>
...
export default function IdHomePageClient() {
  return (
