...
import { buildArticleRouteParam } from "@/lib/article-slug"

function buildIdArticlePath(id: number, title: string): string {
  return `/id/articles/${buildArticleRouteParam(id, title)}`
}
...
export default function IdCategoriesPageClient() {
  ...
  useEffect(() => {
    fetch("/api/categories?language=id")
      .then((res) => res.json())
      .then((data) => {
      ...
      <main className="relative z-10 max-w-7xl mx-auto px-4 py-12 sm:px-6 lg:px-8">
        <div className="mb-12">
          <h1 className="text-4xl md:text-5xl font-black text-white mb-4">
            Jelajahi per <span className="bg-clip-text text-transparent bg-gradient-to-r from-cyan-400 to-blue-400">Kategori</span>
          </h1>
          <p className="text-gray-400 text-lg max-w-2xl">
            Temukan artikel berdasarkan topik. Pilih kategori untuk melihat semua berita terkait.
          </p>
        </div>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-4 gap-8">
            <div className="lg:col-span-1">
              <div className="sticky top-20 space-y-3">
                <h2 className="text-sm font-bold text-cyan-400 uppercase tracking-wider mb-4">Kategori</h2>
                ...
                {filteredCategories.length > DEFAULT_VISIBLE_CATEGORIES && (
                  ...
                  >
                    {showAllCategories ? "Tampilkan Lebih Sedikit" : `Tampilkan Semua (${filteredCategories.length})`}
                  </button>
                )}
              </div>
            </div>
                ...
                <>
                  <div className="mb-8">
                    ...
                    <p className="text-gray-400">
                      {totalArticles} {totalArticles === 1 ? "artikel" : "artikel"} ditemukan
                    </p>
                  </div>
                  ...
                  {loadingArticles ? (
                    ...
                    <>
                      {articlesWithImage.length > 0 && (
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-10">
                          {articlesWithImage.map((article) => (
                            <Link
                              key={article.id}
                              href={buildIdArticlePath(article.id, article.title)}
                            ...
                            >
                              ...
                              <div className="p-5 flex-1 flex flex-col">
                                ...
                                <div className="mt-4 pt-3 border-t border-cyan-500/10">
                                  <span className="text-xs text-cyan-300 font-semibold">Baca Artikel →</span>
                                </div>
                              </div>
                            </Link>
                          ))}
                      ...
                      {articlesWithoutImage.length > 0 && (
                        <div className="mb-12">
                          <h3 className="text-sm font-bold text-cyan-400 uppercase tracking-wider mb-4">Berita Lainnya</h3>
